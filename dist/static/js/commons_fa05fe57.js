...
!function (win) {
  ...
  ,
    ...
    win.console && console.error && console.error('Layui hint: ' + msg);
      ...
      isOpera = typeof opera !== 'undefined' && opera.toString() === '[object Opera]' //内置模块
  ,
      modules = {
  ...
  Layui.prototype.cache = config; //定义模块
  ...
  Layui.prototype.use = function (apps, callback, exports) {
    ...
    apps = typeof apps === 'string' ? [apps] : apps; //如果页面已经存在jQuery1.7+库且所定义的模块依赖jQuery，则不加载内部jquery模块
    ...
    var item = apps[0],
        timeout = 0;
    exports = exports || []; //静态资源host

    config.host = config.host || (dir.match(/\/\/([\s\S]+?)\//) || ['//' + location.host + '/'])[0]; //加载完毕
    ...
    if (!config.modules[item]) {
      var node = doc.createElement('script') //如果是内置模块，则按照 dir 参数拼接模块路径
  ...
  Layui.prototype.link = function (href, fn, cssname) {
    ...
    if (typeof fn !== 'function') return that; //轮询css是否加载完毕
  ...
  Layui.prototype.router = function (hash) {
    ...
    hash = hash.replace(/^#\//, '');
    data.href = '/' + hash;
    hash = hash.replace(/([^#])(#.*$)/, '$1').split('/') || []; //提取Hash结构
  ...
  Layui.prototype.data = function (table, settings, storage) {
    ...
    if (!win.JSON || !win.JSON.parse) return; //如果settings为null，则删除表
  ...
  Layui.prototype.device = function (key) {
    var agent = navigator.userAgent.toLowerCase() //获取版本号
    ,
    ...
    ,
      ...
      ie: function () {
        //ie版本
        return !!win.ActiveXObject || "ActiveXObject" in win ? (agent.match(/msie\s(\d+)/) || [])[1] || '11' //由于ie11并没有msie的标识
        : false;
      }(),
  ...
  Layui.prototype.event = Layui.event = function (modName, events, params, fn) {
    ...
    if (fn) {
      config.event[eventName] = config.event[eventName] || {}; //这里不再对多次事件监听做支持，避免更多麻烦
